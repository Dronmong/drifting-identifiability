...
This **development** campaign is complete through O5. The predeclared
advancement conjunction failed, so O6 was not started and the campaign does
not support a confirmatory performance claim. Results are recorded in
[`OASQDDevelopmentResults.md`](OASQDDevelopmentResults.md). The two LB-QCD
registries and their result artifacts remained sealed and were not used for
OA-SQD selection.
...
The O4-selected primary candidate was frozen as
`oasqd-edge-stop-Q0.08-H25-P50-N2048-K8`. The O5 runner requires this label via
`--primary-candidate`; it cannot select a replacement after seeing O5. The
candidate failed the O5 conjunction (`ED2/QLD = 1.0031`), so there is no O6
candidate.
...
Observed gate outcome: family safety, compute, and divergence passed; the two
aggregate effect gates, per-initialization gate, and event-time gate failed.
The thresholds above are unchanged.
