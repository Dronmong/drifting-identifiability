...
def train_arm(
    ...
    for macro in range(macro_steps):
        ...
        result = transport_then_amortize_step(
            ...
            local_weight=LOCAL_WEIGHT if use_local else 0.0,
    ...
    projection_products = len(target_batches.reshape(-1, exact_atlas.dimension)) * 0
    projection_products += atlas.target_count * direction_count
    projection_products += macro_steps * 2 * PARTICLE_POPULATION * direction_count
    sort_work = (
        macro_steps
        * direction_count
        * PARTICLE_POPULATION
    ...
    if protect:
        ...
        projection_products += macro_steps * 6 * PARTICLE_POPULATION * direction_count
        sort_work += (
            macro_steps
            * direction_count
            * PARTICLE_POPULATION
    ...
    paper_pairs = (
        macro_steps * PARTICLE_POPULATION * (2 * PARTICLE_POPULATION)
        if use_local
        else 0
    )
    ledger: dict[str, float | int] = {
        ...
        + (macro_steps * PARTICLE_POPULATION if use_local else 0),
