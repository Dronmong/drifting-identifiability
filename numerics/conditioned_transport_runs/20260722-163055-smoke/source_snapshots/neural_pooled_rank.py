...
def select_protected_local_weight(
    ...
    *,
    ...
    """Choose the largest one-step local weight safe for total and tail ranks.

    Safety is evaluated against the current particles using the fixed global
    rank assignment.  This is an empirical one-step guard, not a theorem of
    mode preservation or convergence.
    """
    ...
    with torch.no_grad():
        ...
        tails: list[float] = []
        safe: list[bool] = []
        total_limit = total_before * (1.0 + relative_tolerance) + relative_tolerance
        tail_limit = tail_before * (1.0 + relative_tolerance) + relative_tolerance
        for weight in weights:
            ...
            totals.append(total)
            tails.append(tail)
            safe.append(total <= total_limit and tail <= tail_limit)
    accepted = [
