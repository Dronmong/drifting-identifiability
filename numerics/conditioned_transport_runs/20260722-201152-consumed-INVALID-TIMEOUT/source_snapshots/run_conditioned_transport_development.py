    ...
    for arm in ARMS:
        ...
        result["by_arm"][arm] = {
            ...
            "median_online_training_wall_seconds": float(
                np.median(
                    [row["online_training_wall_seconds"] for row in selected]
                )
            ),
            "median_setup_plus_training_wall_seconds": float(
                np.median(
                    [row["setup_plus_training_wall_seconds"] for row in selected]
                )
            ),
            "minimum_mode_coverage": float(
                np.min([row["mode_coverage"] for row in selected])
            ),
...
def main() -> None:
    ...
    for arm in ARMS:
        ...
        artifact.log(
            ...
            f"coverage>={values['minimum_mode_coverage']:.3f} "
