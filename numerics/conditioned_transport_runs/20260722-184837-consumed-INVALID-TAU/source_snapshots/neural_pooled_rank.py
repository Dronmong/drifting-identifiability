...
def projection_tree_representatives(
    points: Tensor, directions: Tensor, representative_count: int
) -> WeightedRepresentativeSet:
    """Compress a support by balanced recursive registered-direction splits."""
    if (
        points.ndim != 2
        or len(points) < 2
    ...
    if count < 1 or count > len(points) or count & (count - 1):
        raise ValueError("representative count must be a power of two within support")
    ...
    clusters = [torch.arange(len(points), device=points.device, dtype=torch.long)]
    for level in range(levels):
        direction = unit_directions[level % len(unit_directions)]
        next_clusters: list[Tensor] = []
        for indices in clusters:
            projected = points[indices] @ direction
            order = torch.argsort(projected, stable=True)
            ordered = indices[order]
            split = len(ordered) // 2
            if split < 1 or split == len(ordered):
                raise AssertionError("balanced representative split became empty")
            next_clusters.extend((ordered[:split], ordered[split:]))
        clusters = next_clusters
    if len(clusters) != count or any(len(indices) < 1 for indices in clusters):
        raise AssertionError("representative tree produced invalid leaves")

    centers = torch.stack([points[indices].mean(dim=0) for indices in clusters])
    multiplicities = torch.tensor(
        [len(indices) for indices in clusters],
        dtype=points.dtype,
        device=points.device,
    )
    assignment = torch.empty(len(points), dtype=torch.long, device=points.device)
    for representative, indices in enumerate(clusters):
        assignment[indices] = representative
    ...
    *,
    ...
    with torch.no_grad():
        ...
        if local_field_builder is not None and local_weight > 0.0:
            local_field = local_field_builder(run_features).detach()
            if local_field.shape != run_features.shape:
        ...
        teacher = (
            run_features
            + particle_step * (global_correction + selected_weight * normalized_local)
        ).detach()
