        ...
        if local_active:
            ...
            if use_representatives:
                ...
                def builder(
                    ...
                    approximate = weighted_normalized_paper_field(
                        run_features,
                        positive_representatives,
                        negative_representatives,
                    )
                    if remaining_representative_audits > 0:
                        dense = torch_paper_field(run_features, positive)
                        denominator = torch.linalg.vector_norm(dense).clamp_min(
                            torch.finfo(dense.dtype).tiny
                        )
                        representative_field_relative_l2.append(
                            float(
                                (
                                    torch.linalg.vector_norm(approximate - dense)
                                    / denominator
                                ).cpu()
                            )
                        )
                        representative_field_cosine.append(
                            float(
                                (
                                    torch.sum(approximate * dense)
                                    / (
                                        torch.linalg.vector_norm(approximate)
