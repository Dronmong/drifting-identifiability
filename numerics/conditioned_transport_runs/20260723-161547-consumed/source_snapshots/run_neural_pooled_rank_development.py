...
class RunArtifact:
    def __init__(
        ...
        *,
        ...
        self.directory = RUN_ROOT / f"{stamp}-{profile.name}{shard_suffix}"
...
def torch_paper_field(
    ...
    return normalized_paper_field(query, positive, tau=tau, mask=mask)
...
def component_core_geometry(
    data: NormalizedTarget,
    *,
    radius_quantile: float = 0.95,
) -> tuple[np.ndarray, np.ndarray] | None:
    """Calibrate component cores from the independent target reference."""
    if data.normalized_centers is None or data.weights is None:
        return None
    centers = np.asarray(data.normalized_centers, dtype=float)
    distances = np.linalg.norm(
        data.reference[:, None, :] - centers[None, :, :], axis=2
    )
    labels = np.argmin(distances, axis=1)
    radii = np.empty(len(centers), dtype=float)
    for component in range(len(centers)):
        selected = distances[labels == component, component]
        if len(selected) < 2:
            return None
        radii[component] = float(np.quantile(selected, radius_quantile))
    return centers, radii


def component_core_diagnostics(
    values: np.ndarray,
    data: NormalizedTarget,
) -> tuple[float, float, float, float]:
    """Report target-calibrated core coverage and bridge/off-core mass."""
    geometry = component_core_geometry(data)
    if geometry is None:
        return (float("nan"),) * 4
    centers, radii = geometry
    ...
    reference_core_mass = np.mean(
        reference_distances <= radii[None, :], axis=0
    )
    output_in_core = output_distances <= radii[None, :]
    output_core_mass = np.mean(output_in_core, axis=0)
    coverage = float(
        np.mean(output_core_mass >= 0.5 * reference_core_mass)
    )
    rare_core_mass = float(output_core_mass[-1])
    nearest = np.argmin(output_distances, axis=1)
    rare_bridge_mass = float(
        np.mean((nearest == len(centers) - 1) & ~output_in_core[:, -1])
    )
    outside_core_mass = float(np.mean(~np.any(output_in_core, axis=1)))
    return coverage, rare_core_mass, rare_bridge_mass, outside_core_mass
...
def evaluate_output(
    ...
    (
        ...
        rare_bridge_mass,
        outside_component_core_mass,
    ) = component_core_diagnostics(values, data)
    return {
        ...
        "component_core_coverage": component_core_coverage,
        "rare_core_mass": rare_core_mass,
        "rare_bridge_mass": rare_bridge_mass,
        "outside_component_core_mass": outside_component_core_mass,
    }
    ...
    for dimension in (2, 4, 8, 16):
        ...
        for arm in ARMS:
            ...
            result["by_dimension"][str(dimension)][arm] = (
                ...
                if selected
                else {"median_ed2": None, "median_heldout_sw1": None}
            )
    return result
...
def main() -> None:
    ...
    if args.shard_count > 1:
        run.log(f"  shard {args.shard_index}/{args.shard_count}")
    ...
    for target in targets:
        ...
        for replication in range(profile.replications):
            ...
            for initialization in profile.initializations:
                ...
                for arm in ARMS:
                    learning_rate_multiplier = LEARNING_RATE_MULTIPLIERS[arm]
                    model, ledger = train_neural_arm(
                        ...
                        kll_atlas,
                        profile,
                        learning_rate=ADAM_LR * learning_rate_multiplier,
                    )
                    with torch.no_grad():
