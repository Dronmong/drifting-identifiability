...
def weighted_representative_field_test() -> dict[str, float]:
    ...
    if maximum_error > 2e-13:
        raise AssertionError(
            f"full weighted field no longer matches dense field: {maximum_error}"
        )
    if (
    ...
    difference = compact - dense
    relative_l2 = float(
        torch.linalg.vector_norm(difference)
        / torch.linalg.vector_norm(dense).clamp_min(torch.finfo(DTYPE).tiny)
    )
    cosine = float(
        torch.sum(compact * dense)
        / (
            torch.linalg.vector_norm(compact)
