...
from neural_pooled_rank import (
    ...
    per_particle_safe_local_weights,
    ...
    select_cross_fitted_local_weight,
    select_protected_local_weight,
    tail_balanced_order,
    tail_priority_scores,
...
def representative_strategy_test() -> dict[str, float | int]:
    """Exercise geometry-aware partitions without changing weighted semantics."""
    rng = np.random.default_rng(SEED + 1701)
    support_size, dimension = 32, 4
    points = torch.tensor(rng.normal(size=(support_size, dimension)), dtype=DTYPE)
    directions = torch.tensor(
        _random_directions(rng, 13, dimension), dtype=DTYPE
    )
    permutation = torch.tensor(rng.permutation(support_size), dtype=torch.long)
    maximum_replay_error = 0.0
    maximum_permutation_error = 0.0
    minimum_full_field_cosine = 1.0
    for strategy in (
        "fixed-level",
        "variance-per-node",
        "radius-priority",
        ...
        first = projection_tree_representatives(
            points,
            directions,
            8,
            strategy=strategy,
            tail_reserve_fraction=0.125,
        )
        replay = projection_tree_representatives(
            points,
            directions,
            8,
            strategy=strategy,
        ...
        maximum_replay_error = max(
            ...
            float(torch.max(torch.abs(first.centers - replay.centers))),
        )
        counts = torch.bincount(first.assignment, minlength=8).to(DTYPE)
        if (
            not torch.equal(counts, first.multiplicities)
            or int(first.multiplicities.sum()) != support_size
            or first.split_count + first.tail_reserve_count != 7
            or first.partition_sort_work <= 0.0
            or first.partition_projection_scalar_products <= 0
        ):
            raise AssertionError(f"{strategy} representative metadata is inconsistent")
        for representative in range(8):
            expected_center = points[first.assignment == representative].mean(dim=0)
            if not torch.allclose(
                expected_center,
                first.centers[representative],
                rtol=0.0,
                atol=2e-15,
            ):
                raise AssertionError(f"{strategy} center is not its leaf mean")

        permuted = projection_tree_representatives(
            points[permutation],
            directions,
            8,
            strategy=strategy,
        ...
        def sorted_centers(value: torch.Tensor) -> np.ndarray:
            array = value.cpu().numpy()
            order = np.lexsort(tuple(array[:, index] for index in reversed(range(dimension))))
            return array[order]
        ...
        maximum_permutation_error = max(
            ...
            float(
                np.max(
                    np.abs(sorted_centers(first.centers) - sorted_centers(permuted.centers))
                )
            ),
        )
        ...
        full_negative = projection_tree_representatives(
            points.flip(0),
            directions,
            support_size,
            strategy=strategy,
        ...
        exact = normalized_paper_field(points.flip(0), points, tau=0.9, mask=True)
        weighted = weighted_normalized_paper_field(
            points.flip(0),
            full_positive,
            full_negative,
        ...
        error = float(torch.max(torch.abs(exact - weighted)))
        if error > 2e-13:
            raise AssertionError(f"{strategy} M=N changed the weighted field: {error}")
        audit = weighted_paper_field_audit(
            points.flip(0),
            points,
            full_positive,
        ...
        minimum_full_field_cosine = min(
            minimum_full_field_cosine, audit["field_cosine"]
        )

    # The first registered direction is deliberately irrelevant. The
    # variance-aware tree must discover the long second-coordinate geometry.
    line = torch.linspace(-5.0, 5.0, 32, dtype=DTYPE)
    anisotropic = torch.stack(
        (0.001 * torch.sin(3.0 * line), line, torch.zeros_like(line)),
        dim=1,
    )
    coordinate_directions = torch.eye(3, dtype=DTYPE)
    fixed = projection_tree_representatives(
        anisotropic, coordinate_directions, 2, strategy="fixed-level"
    )
    adaptive = projection_tree_representatives(
        anisotropic, coordinate_directions, 2, strategy="variance-per-node"
    )
    if adaptive.rms_radius >= 0.6 * fixed.rms_radius:
        raise AssertionError("variance-aware split did not resolve anisotropic geometry")

    tail = projection_tree_representatives(
        points,
        directions,
        8,
        strategy="variance-with-tail-reserve",
        tail_reserve_fraction=0.125,
    )
    singleton_count = int(torch.sum(tail.multiplicities == 1.0))
    if tail.tail_reserve_count != 1 or singleton_count < tail.tail_reserve_count:
        raise AssertionError("tail-reserve strategy did not preserve its extreme point")
    if maximum_replay_error != 0.0 or maximum_permutation_error > 2e-12:
        raise AssertionError(
            "representative strategies are not deterministic/permutation stable: "
            f"replay={maximum_replay_error}, permutation={maximum_permutation_error}"
        )
    return {
        "representative_strategy_replay_max_abs_error": maximum_replay_error,
        "representative_strategy_permutation_max_abs_error": (
            maximum_permutation_error
        ),
        "representative_strategy_full_field_minimum_cosine": (
            minimum_full_field_cosine
        ),
        "variance_tree_anisotropic_radius_ratio": (
            adaptive.rms_radius / fixed.rms_radius
        ),
        "tail_reserved_singletons": singleton_count,
    }
...
def transport_step_and_guard_test() -> dict[str, float | int]:
    ...
    crossfit_reject = select_cross_fitted_local_weight(
        features,
        atlas,
        correction,
        -40.0 * correction,
    ...
    if (
        crossfit_reject.selected_weight != 0.0
        or not crossfit_reject.candidate_safe[0]
    ):
        raise AssertionError(
            "cross-fitted controller did not reject an adversarial local field"
        )
    crossfit_accept = select_cross_fitted_local_weight(
        features,
        atlas,
        torch.zeros_like(correction),
        correction,
        particle_step=0.5,
        tail_relative_tolerance=1.0,
        near_optimal_relative_tolerance=0.0,
    )
    if (
        crossfit_accept.selected_weight <= 0.0
        or crossfit_accept.best_safe_total_loss
        >= crossfit_accept.global_total_loss
    ):
        raise AssertionError(
            "cross-fitted controller rejected an aligned improving local field"
        )
    return {
        ...
        "crossfit_adversarial_selected_weight": crossfit_reject.selected_weight,
        "crossfit_aligned_selected_weight": crossfit_accept.selected_weight,
    }


def transport_rollout_and_local_safety_test() -> dict[str, float | int]:
    rng = np.random.default_rng(SEED + 12)
    torch.manual_seed(SEED + 12)
    population, latent_dimension, dimension = 32, 5, 3
    latents = torch.tensor(rng.normal(size=(population, latent_dimension)), dtype=DTYPE)
    atlas = exact_target_atlas(
        rng.normal(size=(101, dimension)),
        _random_directions(rng, 12, dimension),
        knot_count=53,
    )
    base = TinyGenerator(latent_dimension, dimension).to(dtype=DTYPE)
    model = deepcopy(base)
    result = transport_then_amortize_step(
        ...
        atlas,
        microbatch=8,
        global_rollout_steps=3,
        global_rollout_step_size=0.25,
        rollout_local_after_global=True,
        local_weight=0.0,
    )
    with torch.no_grad():
        manual, displacement, manual_rms = reranked_psqt_rollout(
            result.run_features,
            atlas,
            steps=3,
            step_size=0.25,
        )
    rollout_error = float(torch.max(torch.abs(result.teacher_features - manual)))
    if rollout_error != 0.0 or result.global_rollout_displacement_rms != manual_rms:
        raise AssertionError("reranked rollout disagrees with its direct construction")

    correction = psqt_feature_correction(manual, atlas)
    unsafe_local = -20.0 * correction
    weights = per_particle_safe_local_weights(
        manual,
        atlas,
        unsafe_local,
    )
    assignment = assign_rank_targets(manual, atlas)
    directions = atlas.torch_directions(manual)
    baseline = torch.mean(
        (manual @ directions.T - assignment.assigned_targets).square(), dim=1
    )
    candidate = manual + weights[:, None] * unsafe_local
    selected_loss = torch.mean(
        (candidate @ directions.T - assignment.assigned_targets).square(), dim=1
    )
    maximum_safety_violation = float(
        torch.max(selected_loss - (baseline * (1.0 + 1e-8) + 1e-12))
    )
    if maximum_safety_violation > 2e-12:
        raise AssertionError("per-particle local safety accepted a worsening weight")

    base_order = torch.tensor(rng.permutation(population), dtype=torch.long)
    balanced, tail_count = tail_balanced_order(
        displacement,
        base_order,
        microbatch=8,
        tail_fraction=0.25,
    ...
    tail_indices = torch.topk(
        torch.sum(displacement.square(), dim=1),
        tail_count,
        largest=True,
        sorted=False,
    ).indices
    is_tail = torch.zeros(population, dtype=torch.bool)
    is_tail[tail_indices] = True
    tail_per_batch = [
        int(torch.sum(is_tail[balanced[start : start + 8]]))
        for start in range(0, population, 8)
    ]
    if max(tail_per_batch) - min(tail_per_batch) > 1:
        raise AssertionError("tail-balanced order did not distribute the tail")

    projected_assignment = assign_rank_targets(manual, atlas)
    combined_priority = tail_priority_scores(
        displacement,
        projected_ranks=projected_assignment.ranks,
        projected_extremeness_weight=0.5,
    )
    if (
        combined_priority.shape != (population,)
        or not bool(torch.isfinite(combined_priority).all())
        or float(torch.min(combined_priority)) < 0.0
        or float(torch.max(combined_priority)) > 1.0
    ):
        raise AssertionError("combined tail priority is not a finite percentile score")
    rank_balanced, rank_tail_count = tail_balanced_order(
        displacement,
        base_order,
        microbatch=8,
        tail_fraction=0.25,
    ...
    if (
        rank_tail_count != tail_count
        or not torch.equal(
            torch.sort(rank_balanced).values, torch.arange(population)
        )
    ):
        raise AssertionError("rank-aware tail balance lost a particle")

    diagnostic_model = deepcopy(base)
    diagnostic = transport_then_amortize_step(
        diagnostic_model,
        ...
        rollout_local_after_global=True,
        local_weight=0.0,
        permutation=base_order,
        tail_balanced_amortization=True,
        tail_balance_fraction=0.25,
        tail_projected_extremeness_weight=0.5,
        capture_post_student_features=True,
    )
    if diagnostic.post_student_features is None:
        raise AssertionError("post-student feature capture was not returned")
    post_error = float(
        torch.max(
            torch.abs(
                diagnostic.post_student_features - diagnostic.run_features
            )
        )
    )
    if post_error != 0.0:
        raise AssertionError("zero-rate diagnostic optimizer changed the model")
    recombined_loss = (
        diagnostic.tail_balanced_count * diagnostic.tail_mean_student_loss
        + (population - diagnostic.tail_balanced_count)
        * diagnostic.bulk_mean_student_loss
    ) / population
    if abs(recombined_loss - diagnostic.mean_student_loss) > 2e-14:
        raise AssertionError("tail/bulk losses do not reconstruct the student loss")

    ragged_population = 17
    ragged_displacement = torch.tensor(
        rng.normal(size=(ragged_population, dimension)), dtype=DTYPE
    )
    ragged_order, ragged_tail_count = tail_balanced_order(
        ragged_displacement,
        torch.tensor(rng.permutation(ragged_population), dtype=torch.long),
        microbatch=6,
        tail_fraction=0.25,
    )
    if (
        ragged_tail_count != math.ceil(0.25 * ragged_population)
        or not torch.equal(
            torch.sort(ragged_order).values,
            torch.arange(ragged_population),
        )
    ):
        raise AssertionError("ragged tail balance lost its exact exposure")
    return {
        "rollout_direct_max_abs_error": rollout_error,
        "rollout_displacement_rms": manual_rms,
        "per_particle_safety_max_violation": maximum_safety_violation,
        "tail_balanced_count": tail_count,
        "tail_batch_spread": max(tail_per_batch) - min(tail_per_batch),
        "rank_aware_tail_priority_max": float(torch.max(combined_priority)),
        "post_student_capture_max_abs_error": post_error,
        "tail_bulk_loss_recombination_error": abs(
            recombined_loss - diagnostic.mean_student_loss
        ),
        "ragged_tail_balanced_count": ragged_tail_count,
    }
...
def main() -> None:
    ...
    tests = (
        ...
        transport_rollout_and_local_safety_test,
    )
    for test in tests:
        report.update(test())
