...
class TransportAmortizationResult:
    ...
    tail_mean_student_loss: float
    bulk_mean_student_loss: float
    post_student_features: Tensor | None
    effective_population: int
    student_updates: int
...
def tail_priority_scores(
    displacement: Tensor,
    *,
    projected_ranks: Tensor | None = None,
    projected_extremeness_weight: float = 0.0,
) -> Tensor:
    """Combine displacement and projected-rank extremeness on a common scale.

    A zero extremeness weight exactly preserves the historical displacement
    score.  At positive weight, both components are converted to empirical
    percentile ranks before interpolation, so neither physical units nor the
    number of registered directions determines the balance.
    """
    if displacement.ndim != 2 or not displacement.is_floating_point():
        raise ValueError("displacement must be a floating matrix")
    if (
        not math.isfinite(projected_extremeness_weight)
        or not 0.0 <= projected_extremeness_weight <= 1.0
    ):
        raise ValueError("projected extremeness weight must lie in [0, 1]")
    displacement_scores = torch.sum(displacement.square(), dim=1)
    if projected_extremeness_weight == 0.0:
        return displacement_scores
    population = len(displacement)
    if (
        projected_ranks is None
        or projected_ranks.ndim != 2
        or projected_ranks.shape[0] != population
        or projected_ranks.device != displacement.device
        or projected_ranks.is_floating_point()
        or bool(torch.any(projected_ranks < 0))
        or bool(torch.any(projected_ranks >= population))
    ):
        raise ValueError(
            "projected ranks must be an integer (population, directions) matrix"
        )

    def percentile(values: Tensor) -> Tensor:
        order = torch.argsort(values, stable=True)
        ranks = torch.empty(
            population, dtype=displacement.dtype, device=displacement.device
        )
        ranks[order] = torch.arange(
            population, dtype=displacement.dtype, device=displacement.device
        )
        return ranks / max(1, population - 1)

    normalized_ranks = (
        projected_ranks.to(displacement.dtype) + 0.5
    ) / population
    projected_extremeness = torch.mean(
        torch.abs(2.0 * normalized_ranks - 1.0), dim=1
    )
    displacement_percentile = percentile(displacement_scores)
    extremeness_percentile = percentile(projected_extremeness)
    return (
        (1.0 - projected_extremeness_weight) * displacement_percentile
        + projected_extremeness_weight * extremeness_percentile
    )
...
def tail_balanced_order(
    ...
    *,
    ...
    """Spread the highest-priority particles across student minibatches."""
    if displacement.ndim != 2 or not displacement.is_floating_point():
        raise ValueError("displacement must be a floating matrix")
    population = len(displacement)
    if (
    ...
    scores = tail_priority_scores(
        displacement,
        projected_ranks=projected_ranks,
        projected_extremeness_weight=projected_extremeness_weight,
    )
...
def transport_then_amortize_step(
    ...
    *,
    ...
    tail_projected_extremeness_weight: float = 0.0,
    capture_post_student_features: bool = False,
    ...
    if (
        not math.isfinite(tail_projected_extremeness_weight)
        or not 0.0 <= tail_projected_extremeness_weight <= 1.0
    ):
        raise ValueError("tail projected-extremeness weight must lie in [0, 1]")
    ...
    tail_mask = torch.zeros(
        population, dtype=torch.bool, device=latents.device
    )
    if tail_balanced_amortization:
        projected_ranks = None
        if tail_projected_extremeness_weight > 0.0:
            projected_ranks = assign_rank_targets(teacher, atlas).ranks
        order, tail_balanced_count = tail_balanced_order(
            ...
            projected_ranks=projected_ranks,
            projected_extremeness_weight=(
                tail_projected_extremeness_weight
            ),
        )
        priority = tail_priority_scores(
            teacher - run_features,
            projected_ranks=projected_ranks,
            projected_extremeness_weight=(
                tail_projected_extremeness_weight
    ...
    tail_student_loss_sum = 0.0
    bulk_student_loss_sum = 0.0
    tail_student_count = 0
    bulk_student_count = 0
    for start, stop in ranges:
        indices = order[start:stop]
        optimizer.zero_grad(set_to_none=True)
        prediction = forward_features(latents[indices])
        _validate_features(prediction, atlas)
        per_example_loss = 0.5 * torch.sum(
            (prediction - teacher[indices]).square(), dim=1
        )
        loss = torch.mean(per_example_loss)
        loss.backward()
        optimizer.step()
        detached_loss = per_example_loss.detach()
        student_loss_sum += float(torch.sum(detached_loss).cpu())
        if tail_balanced_count:
            is_tail = tail_mask[indices]
            tail_student_loss_sum += float(
                torch.sum(detached_loss[is_tail]).cpu()
            )
            bulk_student_loss_sum += float(
                torch.sum(detached_loss[~is_tail]).cpu()
            )
            tail_student_count += int(torch.sum(is_tail).cpu())
            bulk_student_count += int(torch.sum(~is_tail).cpu())

    post_student_features = None
    if capture_post_student_features:
        with torch.no_grad():
            post_student_features = torch.cat(
                [
                    forward_features(latents[start:stop])
                    for start, stop in ranges
                ],
                dim=0,
            ).detach()
    ...
    return TransportAmortizationResult(
        ...
        mean_student_loss=student_loss_sum / population,
        tail_mean_student_loss=(
            tail_student_loss_sum / tail_student_count
            if tail_student_count
            else 0.0
        ),
        bulk_mean_student_loss=(
            bulk_student_loss_sum / bulk_student_count
            if bulk_student_count
            else 0.0
        ),
        post_student_features=post_student_features,
