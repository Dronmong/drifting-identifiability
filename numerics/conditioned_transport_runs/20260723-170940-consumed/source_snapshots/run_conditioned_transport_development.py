...
def arm_rollout_configuration(
    ...
    rollout_steps = 1
    if arm.startswith("cta-exact-adaptive-rollout"):
    ...
    rank_balanced = enabled and "rank-balanced" in arm
    return {
        ...
        "tail_projected_extremeness_weight": 0.50 if rank_balanced else 0.0,
    }
...
def train_optimized_paper_arm(
    ...
    *,
    ...
    ledger: dict[str, float | int] = {
        ...
        "tail_projected_extremeness_weight": 0.0,
        "mean_tail_balanced_count": 0.0,
        "mean_student_loss": 0.0,
        "mean_tail_student_loss": 0.0,
        "mean_bulk_student_loss": 0.0,
        "diagnostic_generator_forward_calls": 0,
        "diagnostic_generator_example_evals": 0,
        ...
        "maximum_post_student_rare_core_count": 0,
        "final_post_student_rare_core_count": 0,
        "steps_retaining_teacher_rare_core": 0,
        "mean_teacher_to_post_student_rare_core_ratio": 0.0,
        "maximum_teacher_rare_nearest_count": 0,
        "final_teacher_rare_nearest_count": 0,
        "maximum_run_rare_nearest_count": 0,
        "maximum_post_student_rare_nearest_count": 0,
...
def train_arm(
    ...
    *,
    ...
    tail_balanced_counts: list[int] = []
    student_losses: list[float] = []
    tail_student_losses: list[float] = []
    bulk_student_losses: list[float] = []
    teacher_rare_core_counts: list[int] = []
    run_rare_core_counts: list[int] = []
    post_student_rare_core_counts: list[int] = []
    teacher_rare_nearest_counts: list[int] = []
    run_rare_nearest_counts: list[int] = []
    post_student_rare_nearest_counts: list[int] = []
    ...
    for macro in range(macro_steps):
        ...
        result = transport_then_amortize_step(
            ...
            capture_post_student_features=(
                diagnostic_component_centers is not None
            ),
        )
        ...
        tail_balanced_counts.append(result.tail_balanced_count)
        student_losses.append(result.mean_student_loss)
        tail_student_losses.append(result.tail_mean_student_loss)
        bulk_student_losses.append(result.bulk_mean_student_loss)
        if diagnostic_component_centers is not None:
            ...
            if result.post_student_features is None:
                raise AssertionError(
                    "component retention requested without post-student features"
                )
            distances = torch.cdist(result.post_student_features, centers)
            post_student_rare_nearest_counts.append(
                int(
                    torch.sum(
                        torch.argmin(distances, dim=1) == len(centers) - 1
                    )
                )
            )
            post_student_rare_core_counts.append(
                int(
                    torch.sum(
                        torch.linalg.vector_norm(
                            result.post_student_features
                            - rare_center[None, :],
                            dim=1,
                        )
                        <= rare_radius
    ...
    if tail_balanced_amortization:
        # Top-k tail selection plus one exact permutation of every particle.
        tail_priority_work = macro_steps * (
            PARTICLE_POPULATION * math.log2(PARTICLE_POPULATION)
            + PARTICLE_POPULATION
        )
        training_sort_work += tail_priority_work
        if tail_projected_extremeness_weight > 0.0:
            # One teacher-rank projection/sort supplies projected extremeness.
            training_projection_products += (
                PARTICLE_POPULATION * scheduled_direction_total
            )
            training_sort_work += (
                scheduled_direction_total
                * PARTICLE_POPULATION
    ...
    ledger: dict[str, float | int] = {
        ...
        "diagnostic_generator_forward_calls": (
            macro_steps * chunks
            if diagnostic_component_centers is not None
            else 0
        ),
        "diagnostic_generator_example_evals": (
            macro_steps * PARTICLE_POPULATION
            if diagnostic_component_centers is not None
            else 0
        ),
        ...
        "tail_priority_work": tail_priority_work,
        ...
        "mean_tail_balanced_count": float(np.mean(tail_balanced_counts)),
        "mean_student_loss": float(np.mean(student_losses)),
        "mean_tail_student_loss": float(np.mean(tail_student_losses)),
        "mean_bulk_student_loss": float(np.mean(bulk_student_losses)),
        ...
        "maximum_post_student_rare_core_count": (
            max(post_student_rare_core_counts)
            if post_student_rare_core_counts
            else 0
        ),
        "final_post_student_rare_core_count": (
            post_student_rare_core_counts[-1]
            if post_student_rare_core_counts
            else 0
        ),
        "steps_retaining_teacher_rare_core": (
            sum(
                teacher > 0 and post > 0
                for teacher, post in zip(
                    teacher_rare_core_counts,
                    post_student_rare_core_counts,
                    strict=True,
                )
            )
            if teacher_rare_core_counts
        ...
        "mean_teacher_to_post_student_rare_core_ratio": (
            ...
            if any(count > 0 for count in teacher_rare_core_counts)
            else 0.0
        ),
        ...
        "maximum_post_student_rare_nearest_count": (
            max(post_student_rare_nearest_counts)
            if post_student_rare_nearest_counts
            else 0
        ),
