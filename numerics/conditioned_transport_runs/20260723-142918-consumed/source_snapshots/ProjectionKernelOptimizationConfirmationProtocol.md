...
Post-run clarification (no design or threshold change): the `M = 128` and
`M = 64` runs retained the two predeclared dense field-audit calls per local
arm. Their kernel pairs are excluded from the algorithmic pair ledger but
their execution time remains inside the measured training wall. Any speed
comparison involving those runs is therefore conservative with respect to the
deployed compressed path. The exact pre-outcome protocol text remains in each
artifact's source snapshot.
