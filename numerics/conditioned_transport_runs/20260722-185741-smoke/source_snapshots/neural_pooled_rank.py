...
def projection_tree_representatives(
    ...
    ordered_indices = torch.arange(
        len(points), device=points.device, dtype=torch.long
    )[None, :]
    if levels:
    ...
    if ordered_indices.shape != (count, len(points) // count):
        raise AssertionError("representative tree produced invalid leaves")

    centers = points[ordered_indices].mean(dim=1)
    multiplicities = torch.full(
        (count,),
    ...
    representative_ids = torch.arange(
        count, dtype=torch.long, device=points.device
    )[:, None].expand_as(ordered_indices)
    assignment[ordered_indices.reshape(-1)] = representative_ids.reshape(-1)
