...
def train_arm(
    ...
    *,
    ...
    training_sort_work = (
        scheduled_direction_total * PARTICLE_POPULATION * math.log2(PARTICLE_POPULATION)
    )
    atlas_sort_work = 0.0
    ...
    for target_index, target in enumerate(targets):
        ...
        for execution_position, arm in enumerate(execution_order):
            if arm == PAPER_ARM:
                ...
                arm_representative_count = target_representative_count
                arm_representative_strategy = args.representative_strategy
                arm_tail_reserve_fraction = (
                    args.representative_tail_reserve_fraction
                )
                if arm == "cta-exact-fixed-control":
                    arm_representative_count = 128
                    arm_representative_strategy = "fixed-level"
            ...
            row = {
                ...
                "registered_local_representative_count": (
                    (
                        128
                        if arm == "cta-exact-fixed-control"
                        else target_representative_count
                    )
                ),
                ...
                "registered_representative_strategy": (
                    (
                        ...
                        if arm == "cta-exact-fixed-control"
                            ...
                            if target_representative_count > 0
                            else "dense"
                        )
                    )
                ),
