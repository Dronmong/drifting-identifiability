                    ...
                    if stage == "o2":
                        ...
                        arms.extend([
                            Arm(f"oa-stop-full-{tag}", "oa-full",
                                ...
                                probe_size=probe, target_count=count,
                                one_shot=True),
                            Arm(f"oa-stop-strat-{tag}", "oa-stratified",
                                check_period=check, pulse_length=-1,
                                probe_size=probe, target_count=count,
                            ...
                            Arm(f"oa-once-full-{tag}", "oa-full",
                                ...
                                probe_size=probe, target_count=count,
                                one_shot=True),
                            Arm(f"oa-once-strat-{tag}", "oa-stratified",
                                check_period=check, pulse_length=pulse,
                                probe_size=probe, target_count=count,
                                backward_batch=backward_batch,
                                one_shot=True),
                        ...
                        if stage in ("o4", "o5"):
                            arms.extend([
                                Arm(
                                    f"oasqd-stop-systematic-{tag}",
                                    "oa-stratified",
                                    check_period=check, pulse_length=-1,
                                    probe_size=probe, target_count=count,
                                    backward_batch=backward_batch,
                                    systematic_target=True,
                                    one_shot=True),
                                Arm(
                                    f"oasqd-stop-adaptive-iid-{tag}",
                                    "oa-stratified",
                                    check_period=check, pulse_length=-1,
                                    probe_size=probe, target_count=count,
                                    backward_batch=backward_batch,
                            ...
                            for edge in edge_quantiles:
                                arms.append(Arm(
                                    f"oasqd-edge-stop-Q{edge:.2f}-{tag}",
                                    "oa-stratified",
                                    check_period=check, pulse_length=-1,
                                    probe_size=probe, target_count=count,
                                    backward_batch=backward_batch,
                                    adaptive=True, systematic_target=True,
                                    one_shot=True, edge_quantile=edge))
    return arms
        ...
        if early_hybrid and arm.kind.startswith("oa-"):
            ...
            if (not one_shot_finished and
                ...
                assessment = assess_occupancy(
                    ...
                    edge_quantile=arm.edge_quantile)
...
def summarize(rows: list[dict[str, Any]], stage: str,
              bootstrap_reps: int,
              primary_candidate: str = "") -> dict[str, Any]:
    ...
    if primary_candidate:
        if primary_candidate not in candidates:
            raise ValueError(
                f"primary candidate {primary_candidate!r} is not an "
                f"available OA arm: {candidates}")
        best = primary_candidate
    else:
    ...
    return {
        ...
        "best_candidate": best,
        "candidate_selection": (
            "predeclared" if primary_candidate else "development-minimum"),
        "best_vs_paper_oracle": oracle_ratio,
...
def write_results_md(path: Path, summary: dict[str, Any]) -> None:
    ...
    if summary["stage"] == "o1":
        ...
        candidate_label = (
            "predeclared OA candidate"
            if summary.get("candidate_selection") == "predeclared"
            else "development-selected OA candidate")
        lines.extend([
            f"- {candidate_label}: `{best}`",
        ...
        if best:
            lines.extend(["", "## Candidate details", "", "```json",
...
def main() -> None:
    ...
    parser.add_argument("--edge-quantiles", default="0.25")
    parser.add_argument("--inits", default="missing,concentrated")
    parser.add_argument("--targets", default="")
    parser.add_argument("--arms", default="")
    parser.add_argument(
        "--primary-candidate", default="",
        help=("predeclared arm evaluated by O5 gates; required for O5 and "
              "never selected from O5 outcomes"))
    ...
    if args.reanalyze is not None:
        ...
        summary = summarize(
            rows, stage,
            PROFILES[str(manifest["profile"]["name"])].bootstrap_reps,
            str(manifest.get("primary_candidate", "")))
    ...
    if args.stage == "o5" and not args.primary_candidate:
        parser.error("--primary-candidate is required for O5")
    ...
    if args.stage == "o1":
        ...
        arms = make_arms(
            ...
            edge_quantiles=_parse_numbers(args.edge_quantiles, float))
        ...
        summary = summarize(
            rows, args.stage, profile.bootstrap_reps,
