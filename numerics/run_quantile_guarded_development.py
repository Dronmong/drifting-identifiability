...
SAFETY_PROTOCOL = HERE / "QuantileSafetyGuardProtocol.md"
...
# QGD-v2 is a separate, post-Q1 development suite.  It does not maintain a
# quantile Adam state or force quantile progress.  The three registered arms
# alter only the strength of the pre-update evidence gate and correction cap.
SAFETY_ARMS = (
    Arm("lbqcd", "paper"),
    Arm("qsafe-balanced", "safety"),
    Arm("qsafe-permissive", "safety",
        minimum_gradient_agreement=0.10,
        minimum_ascent_cosine=0.02,
        minimum_signal_to_null=1.25,
        maximum_correction_ratio=0.25),
    Arm("qsafe-strict", "safety",
        minimum_gradient_agreement=0.50,
        minimum_ascent_cosine=0.05,
        minimum_signal_to_null=2.00,
        maximum_correction_ratio=0.15),
)

ARM_SUITES = {"q1": Q1_ARMS, "safety": SAFETY_ARMS}
ARMS = Q1_ARMS
...
def _add_safety_work(total: dict[str, float], diagnostics) -> None:
    total["optimizer_updates"] += float(diagnostics.optimizer_updates)
    total["generator_forward_calls"] += \
        float(diagnostics.generator_forward_calls)
    total["generator_example_evals"] += \
...
def _branch(
        ...
        suffix_targets: np.ndarray, safety_probe_latents: np.ndarray,
        safety_probe_targets: np.ndarray, endpoint_latent: np.ndarray,
    ...
    safety_robust_count = safety_applied_count = 0
    safety_cap_rejected_count = safety_local_rejected_count = 0
    ...
    safety_ascent_cosines: list[float] = []
    safety_signal_ratios: list[float] = []
    diverged = False
    ...
    for offset in range(len(suffix_latents)):
        ...
        elif arm.kind == "safety":
            diagnostics = quantile_safety_step(
                model, latent, positive,
                safety_probe_latents[offset], safety_probe_targets[offset],
                local_kind=arm.local_kind, tau=PAPER_TAU,
                config=SafetyGuardConfig(
                    minimum_gradient_agreement=
                        arm.minimum_gradient_agreement,
                    minimum_ascent_cosine=arm.minimum_ascent_cosine,
                    minimum_signal_to_null=arm.minimum_signal_to_null,
                    maximum_correction_ratio=
                        arm.maximum_correction_ratio))
            _add_safety_work(work, diagnostics)
            active_counts[diagnostics.action] = \
                active_counts.get(diagnostics.action, 0) + 1
            safety_robust_count += int(
                diagnostics.robust_ascent_detected)
            safety_applied_count += int(
                diagnostics.intervention_applied)
            safety_cap_rejected_count += int(diagnostics.cap_rejected)
            safety_local_rejected_count += int(
                diagnostics.local_safety_rejected)
            singular_count += int(diagnostics.singular_candidates)
            if diagnostics.robust_ascent_detected:
                correction_norms.append(
                    diagnostics.correction_metric_norm)
                correction_ratios.append(diagnostics.correction_ratio)
            q_derivatives.append(diagnostics.train_directional_after)
            local_derivatives.append(diagnostics.local_directional_after)
            gradient_cosines.append(diagnostics.gradient_agreement)
            safety_ascent_cosines.append(min(
                diagnostics.train_ascent_cosine,
                diagnostics.probe_ascent_cosine))
            safety_signal_ratios.append(diagnostics.signal_to_null)
        else:
    ...
    row = {
        ...
        "metric": arm.metric,
        "quantile_weight": arm.quantile_weight,
        "minimum_gradient_agreement": arm.minimum_gradient_agreement,
        "minimum_ascent_cosine": arm.minimum_ascent_cosine,
        "minimum_signal_to_null": arm.minimum_signal_to_null,
        "maximum_correction_ratio": arm.maximum_correction_ratio,
        ...
        "projection_active_fraction": (
            ...
            if arm.kind == "guarded" else
            (safety_applied_count / suffix_steps
             if arm.kind == "safety" else 0.0)),
        ...
        "safety_robust_ascent_fraction":
            safety_robust_count / suffix_steps,
        "safety_intervention_fraction":
            safety_applied_count / suffix_steps,
        "safety_cap_rejected_fraction":
            safety_cap_rejected_count / suffix_steps,
        "safety_local_rejected_fraction":
            safety_local_rejected_count / suffix_steps,
        ...
        "p90_correction_metric_norm": (
            ...
            if correction_norms else float("nan")),
        "median_correction_ratio": (
            float(np.median(correction_ratios))
            if correction_ratios else float("nan")),
        "p90_correction_ratio": (
            float(np.quantile(correction_ratios, 0.9))
            if correction_ratios else float("nan")),
        ...
        "median_safety_ascent_cosine": (
            float(np.median(safety_ascent_cosines))
            if safety_ascent_cosines else float("nan")),
        "median_safety_signal_to_null": (
            float(np.median(safety_signal_ratios))
            if safety_signal_ratios else float("nan")),
...
def run_trial_group(task: dict[str, Any]) \
    ...
    arms = tuple(Arm(**value) for value in task.get(
    ...
    safety_probe_latents = np.random.default_rng(base + 10).normal(
        size=(suffix_steps, profile.batch, prefix.latent_dim))
    safety_probe_target_rng = np.random.default_rng(base + 11)
    safety_probe_targets = np.asarray([
        target.sample(profile.batch, safety_probe_target_rng)
        for _ in range(suffix_steps)])
    ...
    for arm in arms:
        row, selection = _branch(
            ...
            suffix_latents, suffix_targets,
            safety_probe_latents, safety_probe_targets, endpoint_latent,
...
def validate_campaign(rows: list[dict[str, Any]],
                      selections: list[dict[str, Any]],
                      tasks: list[dict[str, Any]], profile: Profile,
                      arms: tuple[Arm, ...] = ARMS) -> None:
    """Fail closed on missing, duplicate, non-finite, or miscounted rows."""
    expected = len(tasks) * len(arms)
    if len(rows) != expected or len(selections) != expected:
        raise RuntimeError(
            f"campaign cardinality mismatch: rows={len(rows)}, "
            f"selections={len(selections)}, expected={expected}")
    keys = [
        (str(row["target"]), str(row["init"]), int(row["seed"]),
         str(row["arm"]))
        for row in rows
    ]
    if len(set(keys)) != expected:
        raise RuntimeError("campaign contains duplicate arm rows")
    outcome_fields = (
        "endpoint_ed2", "endpoint_sw1", "endpoint_w2",
        "endpoint_coverage", "endpoint_mass_l1",
        "selected_ed2", "selected_sw1", "selected_w2",
        "selected_coverage", "selected_mass_l1",
        "handoff_ed2", "handoff_sw1", "handoff_w2",
        "handoff_coverage", "handoff_mass_l1",
    )
    for row in rows:
        row_key = (str(row["target"]), str(row["init"]),
                   int(row["seed"]), str(row["arm"]))
        if int(row["diverged"]) == 0 and not all(
                math.isfinite(float(row[field]))
                for field in outcome_fields):
            raise RuntimeError(f"non-finite successful row: {row_key}")
        if int(float(row["work_optimizer_updates"])) != profile.steps:
            raise RuntimeError(
                f"optimizer ledger mismatch for {row['arm']}: "
                f"{row['work_optimizer_updates']} != {profile.steps}")
        if float(row["work_checkpoint_storage_bytes"]) <= 0.0:
            raise RuntimeError("checkpoint storage was not accounted")
        if not 0 <= int(row["selected_step"]) <= profile.steps:
            raise RuntimeError("selected checkpoint lies outside the horizon")
...
def summarize(rows: list[dict[str, Any]],
              candidate_prefix: str | None = None) -> dict[str, Any]:
    baseline = "lbqcd"
    ...
    for arm in arms:
        ...
        mechanism[arm] = {
            ...
            "safety_robust_ascent_fraction": float(np.mean([
                float(row["safety_robust_ascent_fraction"])
                for row in arm_rows])),
            "safety_intervention_fraction": float(np.mean([
                float(row["safety_intervention_fraction"])
                for row in arm_rows])),
            "safety_cap_rejected_fraction": float(np.mean([
                float(row["safety_cap_rejected_fraction"])
                for row in arm_rows])),
            "safety_local_rejected_fraction": float(np.mean([
                float(row["safety_local_rejected_fraction"])
                for row in arm_rows])),
    ...
    if candidate_prefix is None:
        candidate_prefix = ("qsafe-" if any(
            arm.startswith("qsafe-") for arm in arms) else "qgd-paper")
    eligible_candidates = [arm for arm in arms
                           if arm.startswith(candidate_prefix)]
    if not eligible_candidates:
        raise ValueError(f"no registered candidate starts with {candidate_prefix}")
    best = min(eligible_candidates,
               key=lambda arm: selected_ed_ratios[arm])
    if candidate_prefix == "qsafe-":
        checks = {
            "selected_ed2_vs_lbqcd_at_most_0.98":
                selected_ed_ratios[best] <= 0.98,
            "selected_sw1_vs_lbqcd_at_most_0.99":
                selected_sw_ratios[best] <= 0.99,
            "all_family_ed2_ratios_at_most_1.05":
                all(value <= 1.05 for value in
                    family_ratios[best].values()),
            ...
            "intervention_fraction_below_0.20":
                mechanism[best]["safety_intervention_fraction"] < 0.20,
            "cap_rejected_fraction_below_0.10":
                mechanism[best]["safety_cap_rejected_fraction"] < 0.10,
        }
    else:
        checks = {
    ...
    return {
        ...
        "best_registered_candidate": best,
        "best_registered_paper_qgd": (
            best if candidate_prefix == "qgd-paper" else None),
...
def _run_directory(profile: Profile, suite: str) -> Path:
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    stage = "Q2-safety" if suite == "safety" else "Q1"
...
def _write_manifest(path: Path, profile: Profile, workers: int,
                    ...
                    pre_status: str, invariant_report: dict[str, Any],
                    suite: str, arms: tuple[Arm, ...]) -> None:
    protocol_path = SAFETY_PROTOCOL if suite == "safety" else PROTOCOL
    sources = [
        Path(__file__), HERE / "quantile_guarded_drifting.py", REGISTRY,
        PLAN, protocol_path, HERE / "lbqcd.py",
    ...
    manifest = {
        "protocol": ("QGD-development-Q2-safety-v1"
                     if suite == "safety" else "QGD-development-Q1-v1"),
        "suite": suite,
        "profile": asdict(profile),
        "arms": [asdict(arm) for arm in arms],
...
def main() -> None:
    ...
    parser.add_argument("--suite", choices=tuple(ARM_SUITES), default="q1")
    ...
    arms = ARM_SUITES[args.suite]
    registry = load_registry()
    tasks = make_tasks(registry, profile, arms)
    pre_status = git_text("status", "--porcelain")
    run_dir = _run_directory(profile, args.suite)
    ...
    validate_campaign(rows, selections, tasks, profile, arms)
    summary = summarize(
        rows, "qsafe-" if args.suite == "safety" else "qgd-paper")
    ...
    _write_manifest(run_dir, profile, args.workers, len(tasks), wall,
                    pre_status, invariant_report, args.suite, arms)
    print(json.dumps({
        ...
        "best_registered_candidate":
            summary["best_registered_candidate"],
