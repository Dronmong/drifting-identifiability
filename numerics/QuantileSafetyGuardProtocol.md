...
5. Normalize the two gradients in the local Adam dual norm, average them into
   a variance-reduced consensus gradient `g_bar`, and solve the exact
   projection

   ```text
   minimize  ||delta - delta_D||^2_(M_D^-1)
   subject to g_bar dot delta <= 0.
   ```
...
Both batches must independently classify the local proposal as ascent; only
the projection direction is averaged.  This avoids treating the orthogonal
finite-sample noise in each realization as a separate population constraint.
