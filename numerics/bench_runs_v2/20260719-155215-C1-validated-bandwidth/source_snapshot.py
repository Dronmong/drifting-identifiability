...
def C2(profile: Profile) -> Path:
    ...
    for K, d in config["targets"]:
        ...
        for regime in config["tau_regimes"]:
            ...
            run.log(
                ...
                f"resid={residual:.2e} eta*="
                f"{eta_star / tau:.3f}tau" if eta_star is not None else
                f"  K={K} d={d} tau={regime}: maxRe={ev.real.max():+.3e} "
            ...
            for idx, lam in enumerate(ev):
                spectrum_rows.append(
                    (K, d, regime, idx, lam.real, lam.imag, eta_star, residual,
                     generator_pairs, generator_wall)
                )
            ...
            for arm, eta in arms:
                ...
                for seed in range(profile.c2_seeds):
                    ...
                    q0 = rng.normal(size=(N, d)) * target.L
                    result = train(
                        q0,
                        target,
    ...
    run.save_csv(
        ...
        ["K", "d", "tau_regime", "eig_index", "real", "imag", "eta_star",
         "truth_residual", "generator_kernel_pairs", "generator_wall_seconds"],
        spectrum_rows,
    )
    run.save_csv(
        "c2_per_seed.csv",
