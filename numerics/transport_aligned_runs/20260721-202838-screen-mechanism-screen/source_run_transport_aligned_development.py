...
ARMS = ("pqt-B128", "gated-pqt-M1024")
BASELINE_ROWS = {
    "screen": HERE / "lbqcd_runs/20260720-234146-n1-screen/rows.csv",
    "standard": HERE / "lbqcd_runs/20260720-234857-n1-standard/rows.csv",
}
...
def run_trial(task: dict[str, Any]) -> dict[str, Any]:
    ...
    training_batch = 1024 if routed else profile.batch
    target_samples = 0
    sort_work = 0.0
...
def summarize(candidate_rows: list[dict[str, Any]],
              baseline_rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def main() -> None:
    ...
    run_dir = RUNROOT / f"{stamp}-{args.profile}-{args.label}"
    run_dir.mkdir(parents=True, exist_ok=False)
    _write_rows(run_dir / "candidate_rows.csv", rows)
    baseline_path = args.baseline_rows or BASELINE_ROWS.get(args.profile)
