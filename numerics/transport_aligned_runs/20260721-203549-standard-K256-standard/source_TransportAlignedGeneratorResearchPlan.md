...
- `gated-pqt-M1024-f0.70`: use 1024 target samples for the first 70% of
  updates only when the existing audited LB-QCD resolution diagnostic fires,
  then return to 128.  This is the primary high-resolution arm because it
  matches LB-QCD's virtual-batch target-sample schedule.
- `gated-pqt-M1024`: an explicitly sample-heavier diagnostic using 1024 for
  every update on routed targets.  It estimates headroom but cannot be the
  primary comparison.

The first tests architecture alignment at matched target-sample count.  The
second tests maximum effectiveness against the repository's best method under
the same routing and target-sample policy as LB-QCD.  The third isolates
additional target-sample headroom.  None is allowed to reuse evaluation
samples.
...
4. `pqt-B128`;
5. `gated-pqt-M1024-f0.70`;
6. sample-heavier diagnostic `gated-pqt-M1024`;
7. optionally `gated-pqt-M1024-f0.70 + 10% paper suffix` after pure PQT is
   measured.
