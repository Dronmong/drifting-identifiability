...
@dataclass(frozen=True)
class AdamState:
    """Detached Adam state used for objective-specific proposals.

    Arrays are copied on capture and proposal construction.  Consequently a
    proposal can be simulated without mutating either the model or its source
    optimizer state.
    """
...
@dataclass(frozen=True)
class AdamProposal:
    """One non-mutating Adam parameter-displacement proposal."""

    delta: dict[str, np.ndarray]
    preconditioner: dict[str, np.ndarray]
...
def stopgrad_grads(model: TanhMLP, cache: tuple[np.ndarray, ...],
...
def capture_adam_state(model: TanhMLP) -> AdamState:
    """Capture a storage-independent copy of the model's Adam state."""
    return AdamState(
        step_index=int(model.step_index),
        m={name: model.m[name].copy() for name in model.names},
        v={name: model.v[name].copy() for name in model.names})


def copy_adam_state(state: AdamState) -> AdamState:
    """Return a deep copy suitable for an independent objective stream."""
    return AdamState(
        step_index=int(state.step_index),
        m={name: value.copy() for name, value in state.m.items()},
        v={name: value.copy() for name, value in state.v.items()})


def restore_adam_state(model: TanhMLP, state: AdamState) -> None:
    """Install a detached Adam state without changing model parameters."""
    if set(state.m) != set(model.names) or set(state.v) != set(model.names):
        raise ValueError("Adam state parameter names do not match the model")
    model.step_index = int(state.step_index)
    model.m = {name: state.m[name].copy() for name in model.names}
    model.v = {name: state.v[name].copy() for name in model.names}


def adam_proposal(model: TanhMLP, state: AdamState,
                  grads: dict[str, np.ndarray]) -> AdamProposal:
    """Simulate one Adam step without mutating ``model`` or ``state``.

    ``delta`` is the proposed parameter displacement (new minus old), while
    ``preconditioner`` is the positive diagonal multiplier
    ``lr / (sqrt(vhat) + eps)`` used by the proposal.  It is exposed for the
    guarded-update metric and safe first-order fallbacks.
    """
    if set(grads) != set(model.names):
        raise ValueError("gradient parameter names do not match the model")
    if set(state.m) != set(model.names) or set(state.v) != set(model.names):
        raise ValueError("Adam state parameter names do not match the model")
    step = int(state.step_index) + 1
    if step <= 0:
        raise ValueError("Adam proposal step must be positive")
    next_m: dict[str, np.ndarray] = {}
    next_v: dict[str, np.ndarray] = {}
    delta: dict[str, np.ndarray] = {}
    preconditioner: dict[str, np.ndarray] = {}
    for name in model.names:
        gradient = np.asarray(grads[name], dtype=float)
        if gradient.shape != model.params[name].shape:
            raise ValueError(f"gradient shape mismatch for {name}")
        if not np.all(np.isfinite(gradient)):
            raise FloatingPointError("non-finite generator gradient")
        m = BETA1 * state.m[name] + (1.0 - BETA1) * gradient
        v = BETA2 * state.v[name] + (1.0 - BETA2) * gradient * gradient
        mhat = m / (1.0 - BETA1 ** step)
        vhat = v / (1.0 - BETA2 ** step)
        diagonal = ADAM_LR / (np.sqrt(vhat) + ADAM_EPS)
        next_m[name] = m
        next_v[name] = v
        preconditioner[name] = diagonal
        delta[name] = -diagonal * mhat
    return AdamProposal(
        delta=delta,
        preconditioner=preconditioner,
        next_state=AdamState(step, next_m, next_v))


def apply_parameter_delta(model: TanhMLP,
                          delta: dict[str, np.ndarray]) -> None:
    """Apply a named parameter displacement without touching Adam buffers."""
    if set(delta) != set(model.names):
        raise ValueError("parameter displacement names do not match the model")
    for name in model.names:
        update = np.asarray(delta[name], dtype=float)
        if update.shape != model.params[name].shape:
            raise ValueError(f"parameter displacement shape mismatch for {name}")
        if not np.all(np.isfinite(update)):
            raise FloatingPointError("non-finite parameter displacement")
...
def adam_step(model: TanhMLP, grads: dict[str, np.ndarray]) -> None:
    """Apply exactly one update using the optimizer owned by ``model``."""
    # Keep this operation order identical to the historical implementation.
    # The non-mutating ``adam_proposal`` API is intentionally separate because
    # even algebraically equivalent copying/reassociation breaks the bitwise
    # RSR compatibility contract.
...
# Backward-compatible private aliases.  Historical runners import these names;
# keeping aliases avoids changing their source or numerical path.
_stopgrad_grads = stopgrad_grads
...
def select_tau_by_model_lookahead(
        model: TanhMLP, train_latent: np.ndarray,
        train_target: np.ndarray, eval_latent: np.ndarray,
        eval_target: np.ndarray, taus: Iterable[float]) -> TauSelection:
    """Choose bandwidth by one exact cloned-Adam update and held-out W2.

    Candidate models inherit the current parameters, moments, and Adam step.
    Each receives one paper update on the selection-training batch and is
    evaluated on an independent latent/target batch.  No candidate mutates the
    live model and no target labels or mixture parameters are used.
    """
    if train_target.shape != (len(train_latent), 1):
        raise ValueError("lookahead training arrays must be equal 1-D batches")
    if eval_target.shape != (len(eval_latent), 1):
        raise ValueError("lookahead evaluation arrays must be equal 1-D batches")
    scores: dict[float, float] = {}
    kernel_pairs = 0
    generator_evals = 0
    for raw_tau in taus:
        ...
        _, _, work = paper_step(
            candidate, train_latent, train_target, tau)
        predicted = candidate.forward(eval_latent)
        generator_evals += work.generator_example_evals + len(eval_latent)
        kernel_pairs += work.kernel_pairs
        ordered_q = np.sort(predicted[:, 0])
        ordered_p = np.sort(eval_target[:, 0])
        w2 = float(np.mean((ordered_q - ordered_p) ** 2))
        scores[tau] = -w2
    if not scores:
        raise ValueError("at least one bandwidth candidate is required")
    chosen = max(sorted(scores), key=lambda tau: scores[tau])
    return TauSelection(
        ...
        scores=scores,
        kernel_pairs=kernel_pairs,
        generator_example_evals=generator_evals,
    )
