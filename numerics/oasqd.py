...
class OccupancyAssessment:
    ...
    generated_median_quantile: float
...
def assess_occupancy(atlas: QuantileAtlas, generated: np.ndarray, *,
                     ...
                     tolerance: float = 0.0,
                     edge_quantile: float = 0.0) -> OccupancyAssessment:
    ...
    """
    if (ordinary_batch <= 0 or target_count <= 0 or deficit_z <= 0 or
            not 0.0 <= edge_quantile < 0.5):
    ...
    q = counts / n
    generated_median = float(np.median(np.asarray(generated)[:, 0]))
    median_quantile = float(np.searchsorted(
        atlas.ordered[:, 0], generated_median, side="right") /
        atlas.sample_count)
    scores: list[float] = []
    active: list[int] = []
    for j, (p_j, q_j) in enumerate(zip(atlas.masses, q)):
        ...
        if (ordinary_batch * p_j < target_count and
            ...
            active.append(j)
    if (edge_quantile > 0 and
            edge_quantile <= median_quantile <= 1.0 - edge_quantile):
        active = []
    return OccupancyAssessment(
        ...
        generated_median_quantile=median_quantile)
