...
def _trial_steps(arm: Arm, profile: Profile) -> int:
    ...
    if arm.kind in ("rsr", "align", "nrsr", "gated") and arm.budget == "examples":
...
def run_trial(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if arm.kind == "gated":
        ...
        diagnosis = diagnose_quantile_resolution(
    ...
    for step in range(1, total_steps + 1):
        if arm.kind in ("qld", "rsr", "align", "pulse", "nrsr", "gated") and step <= warm_steps:
            use_rsr = (arm.kind in ("rsr", "align", "nrsr") or
                       (arm.kind == "pulse" and
