...
def _trial_steps(arm: Arm, profile: Profile) -> int:
    if arm.kind in ("rsr", "align") and arm.budget == "examples":
...
def run_trial(task: dict[str, Any]) -> dict[str, Any]:
    ...
    for step in range(1, total_steps + 1):
        if arm.kind in ("qld", "rsr", "align") and step <= warm_steps:
            n = profile.batch if arm.kind == "qld" else arm.virtual_batch
...
def summarize(rows: list[dict[str, Any]], stage: str,
              selected_tau: float) -> dict[str, Any]:
    ...
    update_candidates = [arm for arm in candidates
                         if arm.endswith("-u") and
                         ((stage == "n1" and arm.startswith("rsr-")) or
                          (stage == "n2" and arm.startswith("lbqcd-align-")))]
    best = min(update_candidates, key=lambda arm: ratios_vs_paper[arm]) \
        if update_candidates else None
    gate: dict[str, bool] = {}
...
def main() -> None:
    ...
    parser.add_argument("--candidate-taus", default="0.2,0.5,1,2,4")
