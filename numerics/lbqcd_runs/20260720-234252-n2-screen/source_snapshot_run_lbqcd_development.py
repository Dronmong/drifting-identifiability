...
def _trial_steps(arm: Arm, profile: Profile) -> int:
    ...
    if arm.kind in ("rsr", "align", "nrsr", "gated", "gated-align") and arm.budget == "examples":
...
def run_trial(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if arm.kind in ("gated", "gated-align"):
    ...
    for step in range(1, total_steps + 1):
        if arm.kind in ("qld", "rsr", "align", "pulse", "nrsr", "gated", "gated-align") and step <= warm_steps:
            use_rsr = (arm.kind in ("rsr", "align", "nrsr") or
                       (arm.kind == "pulse" and
                        step % arm.pulse_period == 0) or
                       (arm.kind in ("gated", "gated-align") and
            ...
            if arm.kind in ("align", "gated-align") and (
