...
def make_arms(stage: str, virtual_batches: tuple[int, ...],
              ...
              noise_mixes: tuple[float, ...]) -> list[Arm]:
...
def _trial_steps(arm: Arm, profile: Profile) -> int:
    ...
    if arm.kind in ("rsr", "align", "nrsr") and arm.budget == "examples":
...
def run_trial(task: dict[str, Any]) -> dict[str, Any]:
    ...
    for step in range(1, total_steps + 1):
        if arm.kind in ("qld", "rsr", "align", "pulse", "nrsr") and step <= warm_steps:
            use_rsr = (arm.kind in ("rsr", "align", "nrsr") or
                       (arm.kind == "pulse" and
                        step % arm.pulse_period == 0))
