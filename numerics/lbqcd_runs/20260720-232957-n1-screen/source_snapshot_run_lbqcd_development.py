...
def make_arms(stage: str, virtual_batches: tuple[int, ...],
              ...
              selector_period: int,
              pulse_periods: tuple[int, ...]) -> list[Arm]:
...
def run_trial(task: dict[str, Any]) -> dict[str, Any]:
    ...
    for step in range(1, total_steps + 1):
        if arm.kind in ("qld", "rsr", "align", "pulse") and step <= warm_steps:
            use_rsr = (arm.kind in ("rsr", "align") or
                       (arm.kind == "pulse" and
...
def main() -> None:
    ...
    arms = make_arms(args.stage, virtual_batches, warm_fractions, budgets,
                     args.selector_period, pulse_periods)
