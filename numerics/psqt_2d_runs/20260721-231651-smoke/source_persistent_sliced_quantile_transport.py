...
class PersistentSlicedQuantileTransport:
    ...
    def _batch_quantiles(self, target: np.ndarray) -> np.ndarray:
        ...
        return np.vstack([
            empirical_quantiles(projected[:, ell], self.grid)
            for ell in range(len(self.directions))
        ])

    def _reconstruct_once(self) -> None:
        n = len(self.particles)
        count = len(self.directions)
        probabilities = midpoint_grid(n)
        projected = self.particles @ self.directions.T
        desired = np.empty_like(projected)
        for ell in range(count):
            order = np.argsort(projected[:, ell], kind="stable")
            ordered_target = np.interp(
                probabilities, self.grid, self.target_quantiles[ell])
            desired[order, ell] = ordered_target
