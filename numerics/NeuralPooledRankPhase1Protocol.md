...
**Status:** amended after the registered optimization-repair screen, frozen,
and executed  
**Date:** 2026-07-22  
...
The standard matrix may be partitioned by target index for CPU execution.
Sharding is purely an orchestration transform: each target retains the same
registered seeds, budgets, data, arms, and evaluation. A merged artifact is
valid only after verifying complete shard-index coverage, registry and file
hashes, disjoint output/reference keys, KLL payload hashes, the full target
set, and all 672 expected rows.
