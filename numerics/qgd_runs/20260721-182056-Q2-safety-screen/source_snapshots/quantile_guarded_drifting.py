...
def project_consensus_nonascent(
        consensus_gradient: np.ndarray, local_delta: np.ndarray,
        metric: np.ndarray,
        config: SafetyGuardConfig = SafetyGuardConfig()) \
        -> tuple[np.ndarray, str, int]:
    """Project onto non-ascent for a variance-reduced consensus gradient."""
    config.validate()
    gradient = np.asarray(consensus_gradient, dtype=float).ravel()
    base = np.asarray(local_delta, dtype=float).ravel()
    diagonal = np.asarray(metric, dtype=float).ravel()
    if not (gradient.shape == base.shape == diagonal.shape):
        raise ValueError("consensus projection vectors must have equal shapes")
    if not all(np.all(np.isfinite(value)) for value in
               (gradient, base, diagonal)):
        raise FloatingPointError("consensus projection input is non-finite")
    diagonal = np.clip(
        diagonal, config.metric_floor, config.metric_ceiling)
    directional = float(np.dot(gradient, base))
    if _feasible(directional, 0.0, config.feasibility_tolerance):
        return base.copy(), "none", 0
    denominator = float(np.sum(gradient * gradient * diagonal))
    scale = max(1.0, float(np.linalg.norm(gradient)) ** 2)
    if (not math.isfinite(denominator) or
            denominator <= config.singular_tolerance * scale):
        return np.zeros_like(base), "zero-fallback", 1
    candidate = base - diagonal * gradient * (directional / denominator)
    if not _feasible(float(np.dot(gradient, candidate)), 0.0,
                     config.feasibility_tolerance):
        raise AssertionError("consensus projection is infeasible")
    return candidate, "consensus", 0
...
def quantile_safety_step(
    ...
    intervention = False
    if robust:
        train_dual_norm = _dual_metric_norm(train_flat, metric)
        probe_dual_norm = _dual_metric_norm(probe_flat, metric)
        consensus = (
            train_flat / max(train_dual_norm, config.singular_tolerance) +
            probe_flat / max(probe_dual_norm, config.singular_tolerance))
        candidate, active_set, singular = project_consensus_nonascent(
            consensus, local_delta, metric, config)
    ...
    if intervention:
        ...
        applied = local_delta
        # Retain the rejected candidate's correction diagnostics.  The action
        # and intervention flag distinguish this attempted correction from an
        # applied one and make an over-tight cap auditable.
...
def invariant_tests(log=print) -> dict[str, Any]:
    ...
    consensus_projected, consensus_label, consensus_singular = \
        project_consensus_nonascent(
            np.asarray([1.0, 1.0]), np.asarray([1.0, -0.5]),
            np.ones(2), safety_config)
    np.testing.assert_allclose(
        consensus_projected, np.asarray([0.75, -0.75]), atol=1e-12)
    if consensus_label != "consensus" or consensus_singular != 0:
        raise AssertionError("consensus safety projection failed")
    checks.append("safety_projection_consensus")
