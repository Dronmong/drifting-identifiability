...
def _run_branch(
    ...
    trajectory_steps = _trajectory_steps(profile,
                                         int(round(PREFIX_FRACTION *
                                                   profile.steps)))
    label_event: int | None = next((
        int(row["step"]) for row in trajectory
        if (float(row["ed2"]) <= 0.25 * initial_metrics["ed2"] and
            float(row["sw1"]) <= 0.25 * initial_metrics["sw1"])), None)
    occupancy_event: int | None = next((
        int(row["step"]) for row in trajectory
        if (target.has_components and
            row["all_reportable_components_reached"] == 1.0 and
            float(row["mass_l1"]) <= 0.25)), None)
    prefix_best = min(trajectory, key=lambda row: float(row["ed2"]),
                      default=None)
    best_ed2 = (float(prefix_best["ed2"])
                if prefix_best is not None else float("inf"))
    best_step = (int(prefix_best["step"])
                 if prefix_best is not None else start_step)
    ...
    if diverged:
        ...
        endpoint = {key: final_trajectory[key] for key in (
            "ed2", "sw1", "quantile_cdf_error", "output_mean",
            "output_std", "output_q01", "output_q10", "output_q50",
            "output_q90", "output_q99", "mass_l1",
...
def run_trial_group(task: dict[str, Any]) \
    ...
    for arm in ARMS:
        if arm.standalone_paper:
            ...
            arm_prefix_work = dict(initial_work)
            arm_prefix_work["metric_generator_forward_calls"] = 1.0
            arm_prefix_work["metric_generator_example_evals"] = float(
                profile.eval_size)
            arm_prefix_work["metric_target_samples"] = float(
                profile.eval_size)
        else:
            start_model = prefix
...
def hierarchical_ratio_bootstrap(
        rows: list[dict[str, Any]], arm: str, reference: str,
        metric: str, reps: int, seed: int) -> dict[str, float]:
    """Resample targets, then paired seeds within target/initialization."""
    primary = [row for row in rows if row["init"] in PRIMARY_INITS]
    targets = sorted({str(row["target"]) for row in primary})
    lookup = {
        (str(row["target"]), str(row["init"]), int(row["seed"]),
         str(row["arm"])): float(row[metric])
        for row in primary
    }
    seed_lookup = {
        (target, init): sorted({int(row["seed"]) for row in primary
    ...
    for replicate in range(reps):
        ...
        for target in sampled_targets:
            for init in PRIMARY_INITS:
                available = seed_lookup[(str(target), init)]
                sampled_seeds = rng.choice(
                    available, size=len(available), replace=True)
                candidate = np.median([
                    lookup[(str(target), init, int(s), arm)]
                    for s in sampled_seeds])
                baseline = np.median([
                    lookup[(str(target), init, int(s), reference)]
                    for s in sampled_seeds])
                log_ratios.append(math.log(
                    max(float(candidate), 1e-300) /
                    max(float(baseline), 1e-300)))
        statistics[replicate] = math.exp(float(np.mean(log_ratios)))
    lo, hi = np.quantile(statistics, [0.025, 0.975])
    return {
...
def summarize(rows: list[dict[str, Any]], profile: Profile) -> dict[str, Any]:
    ...
    numerically_safe = [arm for arm in eligible if all(
        int(row["diverged"]) == 0 and int(row["floor_activations"]) == 0
        for row in primary if row["arm"] == arm)]
    connected_cells = sorted({str(row["cell"]) for row in primary
                              if str(row["family"]).startswith("connected")})
    connected_ed2_ratios = {
        arm: {
            cell: lookup[(cell, arm)] /
            max(lookup[(cell, "qld-v1")], 1e-300)
            for cell in connected_cells
        }
        for arm in eligible
    }
    safe = [arm for arm in numerically_safe if all(
        value <= 1.10 for value in connected_ed2_ratios[arm].values())]
    ranked = sorted(safe, key=lambda arm: (ratios[arm], sw_ratios[arm], arm))
    bootstrap = {
        arm: hierarchical_ratio_bootstrap(
            rows, arm, "qld-v1", "ed2", profile.bootstrap_reps,
            20260721 + index)
        for index, arm in enumerate(ranked)
    }
    return {
        ...
        "connected_control_ed2_ratios": connected_ed2_ratios,
        "hierarchical_ed2_bootstrap_vs_qld": bootstrap,
        "divergence_counts": {
