...
PROFILES = {
    "smoke": Profile(
        "smoke", 40, 1, 32, 32, 512, 128, 16, 32, 60, 0.5,
        "smoke"),
    "screen": Profile(
        "screen", 300, 5, 64, 64, 1024, 256, 32, 64, 100, 0.5,
        "all"),
}
...
def summarize(rows: list[dict], particles: int) -> dict:
    ...
    deployable = [
        arm for arm in arms
        if arm.startswith("reservoir-") or arm.startswith("kll-style-")
    ]
    deployable_scores = {
        arm: ratio(arm, "historical-online-psqt", "ed2")
    ...
    gates = {
        ...
        "aggregate_ed2_better_than_historical": (
            ratio(selected, "historical-online-psqt", "ed2") < 1.0),
        "aggregate_sw1_better_than_historical": (
            ratio(selected, "historical-online-psqt", "heldout_sw1") < 1.0),
    ...
    for arm in arms:
        arm_summary[arm] = {
            "ed2_vs_historical": ratio(
                arm, "historical-online-psqt", "ed2"),
            "ed2_vs_paper": ratio(arm, "paper-tau1", "ed2"),
            "sw1_vs_historical": ratio(
    ...
    return {
        ...
        "selected_ed2_ratio_vs_historical": ratio(
            selected, "historical-online-psqt", "ed2"),
        "selected_ed2_ratio_vs_paper": ratio(
            selected, "paper-tau1", "ed2"),
        "selected_sw1_ratio_vs_historical": ratio(
            selected, "historical-online-psqt", "heldout_sw1"),
        "exact_ed2_ratio_vs_historical": ratio(
            "exact-pooled-ceiling", "historical-online-psqt", "ed2"),
        "exact_ed2_ratio_vs_paper": ratio(
        ...
        "all_decision_gates_pass": all(gates.values()),
...
def main() -> None:
    ...
    artifact.log(
        ...
        f"`{summary['selected_ed2_ratio_vs_historical']:.4f}`")
    artifact.log(
        "- ED2 ratio / selected paper: "
        f"`{summary['selected_ed2_ratio_vs_paper']:.4f}`")
    artifact.log(
        "- held-out SW1 ratio / historical online PSQT: "
        f"`{summary['selected_sw1_ratio_vs_historical']:.4f}`")
    artifact.log(
        "- exact-pooled ED2 ratio / historical online PSQT: "
        f"`{summary['exact_ed2_ratio_vs_historical']:.4f}`")
    artifact.log(
        "- exact-pooled ED2 ratio / selected paper: "
        f"`{summary['exact_ed2_ratio_vs_paper']:.4f}`")
