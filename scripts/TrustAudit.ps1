...
$allowedPaperAxioms = @(
  ...
  'equation_32_grouped_zero_drift',
  'zero_drift_coefficient_minors',
  'meanShiftInteractionKernel_antisymmetric',
...
if (-not (Test-Path -LiteralPath $hashManifest)) {
  ...
  $manifestLine = (Get-Content -LiteralPath $hashManifest -Raw).Trim()
...
Write-Host "Trust audit passed: $($leanFiles.Count) Lean files checked; paper axiom boundary unchanged."
