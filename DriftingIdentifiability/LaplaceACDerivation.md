...
- L9-finiteness: a hypothesis for general a.c.; automatic for analytic densities.
  The arbitrary finite breakpoint induction is now formalized.  Primitive/FTC
  and W-boundedness fields for upward crossings are generated by
  `LaplaceACUpwardCrossingCertificate.of_regular`, and
  `LaplaceACAlternatingChain.cons_regular` wires that constructor into the
  recursive chain.  The remaining Lean polish is automatic extraction of the
  local numeric witnesses (`leftNear/rightNear`, `δ`, `L`, and one-sided signs)
  from higher-level simple-zero hypotheses.  It is not a new analytic identity,
  final gate, or L9 combinatorial lemma.
